...
class Qwen3Attention(nn.Module):
    def __init__(
        ...
        assert self.total_num_heads % tp_size == 0, "num_heads must be divisible by tp_size"
...
class Qwen3DecoderLayer(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.self_attn = Qwen3Attention(
            ...
            max_positions=config.hf_config.max_position_embeddings, 
            rms_norm_eps=config.hf_config.rms_norm_eps, 
            qkv_bias=getattr(config.hf_config, 'attention_bias', True),
            head_dim=getattr(config.hf_config, 'head_dim', None), 
            rope_theta=getattr(config.hf_config, 'rope_theta', 10000), 
            rope_scaling=getattr(config.hf_config, 'rope_scaling', None), 
        )
        self.mlp = Qwen3MLP(
            hidden_size=config.hf_config.hidden_size, 
            intermediate_size=config.hf_config.intermediate_size, 
            hidden_act=config.hf_config.hidden_act
        )
...
class Qwen3Model(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.layers = nn.ModuleList(
            [Qwen3DecoderLayer(config) for _ in range(config.hf_config.num_hidden_layers)]
        )
        self.norm = RMSNorm(config.hf_config.hidden_size, eps=config.hf_config.rms_norm_eps)
